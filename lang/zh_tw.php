<?php
	...
	檢視原始碼是個 feature 不是個 bug，感謝您對本活動有興趣。

	如果您自願成為全能網頁改造王，請參考以下新版 csszengarden.com 的作品投稿方針：

	- 我要用 CSS3 可以嗎？當然可以！若有必要，請替 **全部** 的瀏覽器加上參數的 prefix。
	- 使用 responsive web design (可適應性網頁設計)；您可以在不同的螢幕如手機平板上測試您的排版。
	- 網頁測試的基本要求：IE9+、最新版本的 Chrome/Firefox/Safari、以及 iOS/Android
	- 在不支援某些功能的瀏覽器上，我們仍鼓勵您提供優雅的降格畫面。
	- 使用 class 做樣式，不要用 ID。
	- 您也可以使用網頁字型，但請確認您有權利分享字型給瀏覽網頁的使用者。

	另外也提供幾個建立 CSS 檔案的小技巧：

	- 使用 :first-child, :last-child, :nth-child 來取得沒有加上 class 的元素。
	- 使用 ::before 與 ::after 來建立 pseudo element 加上額外的花樣。
	- 使用多重背景圖片，背景可以無限制的添加在任何的元素上。
	- 若您仍有圖片取代文字的需求，請使用 Kellum Method 做文字取代。 http://goo.gl/GXxdI
	...
	$intro = array(
		...
		"preamble-h3" => '開悟之路',
		"preamble-p1" => '各家瀏覽器的專屬標籤、低相容度的<abbr title="Document Object Model">DOM</abbr>，或者是缺乏完整支援的<abbr title="Cascading Style Sheets">CSS</abbr>等遺跡，如今都散落在闇黑而荒涼的道路上。',
		...
		"preamble-p3" => 'CSS Zen Garden「樣式表禪意花園」邀請您放鬆身心，冥想專家們的重要課題。讓我們以清晰的心靈之眼凝視，並且用嶄新、充滿活力的方式學習這項受人尊敬的古老技術，達到天人合一的境界。',
	);
...
90% 以上的人們使用）。',
		...
		"requirements-p4" => '這是個示範，也是學習用的練習題。你可以保留圖片的完整著作權（有限度的例外，請參考<a href="' . $zenUrls["zen-guidelines"] . '">投稿方針</a>），但我們要求你用<a href="' . $zenUrls["zen-license"] . '" title="檢視Zen Garden的版權聲明。">與本網站相同</a>的創用CC授權釋出您的<abbr title="Cascading Style Sheets">CSS</abbr>作品，讓其他人可以從中學習。',
		"requirements-p5" => '由<a href="' . $zenUrls["zen-credits-creator"] . '">Dave Shea</a>提供。頻寬由<a href="' . $zenUrls["zen-credits-hosting"] . '">mediatemple</a>慷慨贊助。現正販售中：<a href="' . $zenUrls["zen-credits-book"] . '">Zen Garden之書</a>',
	);
	...
	$footer = array(
		...
		"zen-github-title" => "在GitHub建立本站的分支",
		"zen-github-text" => "GH",
	);
